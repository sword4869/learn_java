- [如何创建线程池](#如何创建线程池)
- [(Executors创建)线程池的种类有哪些](#executors创建线程池的种类有哪些)
- [为什么不建议用Executors创建线程池](#为什么不建议用executors创建线程池)
- [说一下线程池的核心参数（线程池的执行原理知道嘛）](#说一下线程池的核心参数线程池的执行原理知道嘛)
- [ThreadPoolExecutor的4个构造函数](#threadpoolexecutor的4个构造函数)
- [ThreadPoolExecutor 添加任务（工作流程）](#threadpoolexecutor-添加任务工作流程)
- [线程池的拒绝策略](#线程池的拒绝策略)
- [线程池中有哪些常见的阻塞队列](#线程池中有哪些常见的阻塞队列)
- [阻塞队列是干什么的](#阻塞队列是干什么的)
- [ArrayBlockingQueue的LinkedBlockingQueue区别](#arrayblockingqueue的linkedblockingqueue区别)
- [如何确定核心线程数](#如何确定核心线程数)
- [线程池使用场景（你们项目哪里用到了多线程）](#线程池使用场景你们项目哪里用到了多线程)
  - [案例一（es数据批量导入）](#案例一es数据批量导入)
  - [案例二（数据汇总）](#案例二数据汇总)
  - [案例二（异步调用）](#案例二异步调用)
