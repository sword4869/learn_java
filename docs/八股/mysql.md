...
树型表的标记字段是parentid，表示父结点的id。

查询一个树型表的方法:
- 当层级固定时可以用表的**自链接**进行查询。
- 如果想灵活查询每个层级可以使用mysg|**递归**方法，使用with RECURSIVE实现。
...
![alt text](https://cdn.jsdelivr.net/gh/sword4869/pic1@main/images/202407112029087.png)

登陆，校验用户名和密码，只需统计count结果是0还是大于0.

`' or '1'='1`

![alt text](https://cdn.jsdelivr.net/gh/sword4869/pic1@main/images/202407112029088.png)

预编译sql，将整个字符串作为一个参数传入，避免了sql注入问题。

![alt text](https://cdn.jsdelivr.net/gh/sword4869/pic1@main/images/202407112029089.png)
...
聚集索引主要是指**数据与索引放到一块**，B树的叶子节点保存了**整行数据**

非聚集索引值（二级索引）的是**数据与索引分开存储**，B树的叶子节点保存对应的**索引列和聚集索引**

- 聚集索引必须有, 而且只有一个；非聚集索引可以有多个。
...
**在一颗索引树就能获取查询所需的所有列数据，是覆盖索引，否则就是回表查询**。

![alt text](https://cdn.jsdelivr.net/gh/sword4869/pic1@main/images/202407112029090.png)

【覆盖索引的情况】：

（1）聚集索引必是覆盖索引。因为B树的叶子节点保存了整行数据。
        
- where聚集索引: `select *` 都行

（2）非聚集索引中的一种情况，查询列要被所建的索引覆盖。因为B树的叶子节点保存对应的非聚集索引列和聚集索引
- where普通索引: select 索引列、聚集索引列
- where联合索引: select 多个索引列、聚集索引列
...
explain的输出结果Extra字段为Using index时，走覆盖索引；为Null时，走回表查询。
...
insert into t1 value (4, 3, 1, 1, 'd'), (1, 1, 1, 1, 'a'), (8, 8, 8, 8, 'h'), (2, 2, 2, 2, 'b'), (5, 2, 3, 5, 'e'), (3, 3, 2, 2, 'c'), (7, 4, 5, 5, 'g'), (6, 6, 4, 4, 'f');

create index idx_t1_bcd on t1(b,c,d); -- 创建复合索引


-- 用到联合索引，所以在存储引擎层就利用where条件找到了聚集索引，然后回表，再取出查询的列，所以server层是不需要再使用where条件过滤的；
explain select * from t1 where b = 1; -- key是idx_t1_bcd, extra是null。回表
explain select e from t1 where b = 1; -- 一样

explain select b from t1 where b = 1; -- Using index
explain select b,c from t1 where b = 1; -- Using index
explain select b,d from t1 where b = 1; -- Using index
explain select b,c,d from t1 where b = 1; -- Using index
explain select a,b,c,d from t1 where b = 1; -- Using index

explain select c from t1 where d = 1; -- Using where; Using index
explain select * from t1 WHERE b = 1 and d = 1; -- Using index condition
```
...
-- a b 生效: idx_a_b_c有顺序，select查询中where的前后顺序无关，存在就行。
...
![alt text](https://cdn.jsdelivr.net/gh/sword4869/pic1@main/images/202407112029091.png)
...
提前过滤：在没有 ICP 的情况下，存储引擎根据索引来查找记录，服务器再根据where条件来过滤记录；在支持ICP优化后，MySQL 服务器将这部分 WHERE 条件下推给存储引擎。存储引擎判断是否可以进行where条件过滤再进行索引查询。

条件：非覆盖索引、回表查询、只对where中的条件是索引字段。

效果：对索引中包含的字段先做判断，过滤掉不符合条件的记录，减少回表次数。

![alt text](https://cdn.jsdelivr.net/gh/sword4869/pic1@main/images/202407112029092.png)
...
1. master 将数据变更写入二进制日志binlog
2. slave 读取 master 的 binlog，写入到自己的中继日志relay log中。
3. slave 重放 relay log 中事件，将数据变更。
...
- bin log：主从同步用的。记录的DDL和DML语句，不包括查询（select、show）语句。
