...
## 18. String、StringBuffer与StringBuilder的区别？

## 19. equals()既然已经能实现对比的功能了，为什么还要hashCode()呢？

因为重写的equals（）里一般比较的比较全面比较复杂，这样效率就比较低，而利用hashCode()进行对比，则只要生成一个hash值进行比较就可以了，效率很高。

## 20. 为什么在重写 equals 方法的时候需要重写 hashCode 方法?

## 21. hashCode()既然效率这么高为什么还要equals()呢？
...
## 22. 为什么要重写 hashcode 和 equals 方法？
...
---
## 32. JVM的内存结构
## 33. JVM怎么判断一个对象没有被使用（引用计数法，可达性算法）
## 34. 堆空间的基本结构（伊甸园，幸存区from/to，老年代）
---
## 35. 脏读？
...
## 36. sql语言怎么实现乐观锁和悲观锁（for update，version版本控制）
## 37. 数据库的事务级别
## 38. MySQL的索引
## 39. MySQL的日志文件
---
...
## 45. redis+token（redis做登录的话，其中存了什么东西）
...
## 48. 用户登录是否有过期时间，再次登录时，过期时间是顺延还是继续计时（顺延）
## 49. cookie和session的区别（位置，安全性，生命周期，保存数据类型）
## 50. cookie中存了什么样的信息，可以判断用户有否处于登录态（Session ID）
## 51. cookie被伪造？会导致什么问题，怎么解决
