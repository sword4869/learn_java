...
- [参数](#参数)
  - [get请求](#get请求)
  - [post、delete、put请求](#postdeleteput请求)
  - [其他](#其他)
...
参数在url里，`http://localhost/user/1?showDetail=true`


- 参数名不一致：`@RequestParam(value = "name")`, value可省略。
- 请求参数可选。用 `@RequestParam(required = false)`
- 参数默认值 `@RequestParam(defaultValue = "1")`

    ```java
    // http://localhost:8080/user?ids=1,2
