...
- [3. Arrays](#3-arrays)
- [4. 转化](#4-转化)
  - [4.1. 把数组转换为字符串](#41-把数组转换为字符串)
  - [4.2. 集合和基本类型数组](#42-集合和基本类型数组)
  - [4.3. 集合和包装类数组](#43-集合和包装类数组)
    - [4.3.1. 数组转集合](#431-数组转集合)
    - [4.3.2. 集合转数组](#432-集合转数组)
  - [4.4. 包装类](#44-包装类)
- [5. 拷贝数组](#5-拷贝数组)
- [6. sort](#6-sort)
  - [6.1. 默认升序](#61-默认升序)
...
注意：
- 没有`// return {1, 2, 3};`，也不能是`// return int[]{1,2,3};`，也不能是`// return new int[3]{1, 2, 3};`，java只能是`return new int[][]{1,2,3};`
...
// int[]不行，Integer[] 可以
int[] arr = new int[] { 1, 2, 3, 4, 5, 6 };
List<Integer> list = Arrays.asList(arr);    // error
List<Integer> list2 = List.of(arr);         // error
```
