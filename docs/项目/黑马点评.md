- [1. 短信登陆](#1-短信登陆)
  ...
  - [1.4. QA](#14-qa)
    ...
    - [1.4.2. 为什么使用双拦截器而不是一个](#142-为什么使用双拦截器而不是一个)
    - [1.4.3. 为什么登录拦截器的拦截是设置401状态码？](#143-为什么登录拦截器的拦截是设置401状态码)
    - [1.4.4. 为什么刷新拦截器清空UserHolder](#144-为什么刷新拦截器清空userholder)
    - [1.4.5. logout优化](#145-logout优化)
- [2. 优惠券秒杀](#2-优惠券秒杀)
  - [2.1. 全局唯一 id](#21-全局唯一-id)
  - [2.2. 秒杀](#22-秒杀)
  - [2.3. 库存超卖](#23-库存超卖)
  - [2.4. 一人一单](#24-一人一单)
    - [2.4.1. synchronized锁细节](#241-synchronized锁细节)
    - [2.4.2. 分布式锁](#242-分布式锁)
      - [2.4.2.1. 不可重入的Redis分布式锁](#2421-不可重入的redis分布式锁)
      - [2.4.2.2. 可重入的Redisson分布式锁](#2422-可重入的redisson分布式锁)
      - [2.4.2.3. multiLock](#2423-multilock)
  - [2.5. 秒杀业务的优化：基于阻塞队列的异步秒杀](#25-秒杀业务的优化基于阻塞队列的异步秒杀)
...
### 1.4.2. 为什么使用双拦截器而不是一个

解决只有一个拦截器则无法刷新的问题。

只有一个拦截器，则无法对不需要拦截的路径生效，从而无法刷新redis中User有效期。

### 1.4.3. 为什么登录拦截器的拦截是设置401状态码？
